    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-[1300px] mx-auto px-4 py-5 grid grid-cols-[580px_1fr] gap-6">
          ...
          <div className="bg-white rounded-lg shadow-sm overflow-hidden mb-3">
            <div className="relative" style={{ paddingTop: '66%' }}>
              ...
              {product.badges?.map(b => {
                ...
                return <span key={b} className={`absolute left-3 top-3 ${cls} text-white text-[12px] font-black px-2.5 py-1 rounded-sm`}>{label}</span>;
              })}
            </div>
          </div>
            ...
            {tab === 'Avis' && (
              <div className="space-y-4">
                <div className="flex items-center gap-6 pb-4 border-b border-gray-100">
                  <div className="text-center"><div className="font-['Barlow_Condensed'] text-[48px] font-black text-[#0d1b2a]">{product.rating}</div><RatingStars rating={product.rating} /><div className="text-[12px] text-gray-400">{product.reviews} avis</div></div>
        ...
        <div className="space-y-4">
          ...
          {/* Supplier card */}
          <div className="bg-white rounded-lg shadow-sm p-5">
            <div className="flex items-center gap-3 mb-4"><img src={detail.supplier.logo} alt="" className="w-12 h-12 rounded object-cover" /><div><div className="text-[13px] font-black text-[#0d1b2a]">{detail.supplier.name}</div><div className="text-[11px] text-gray-400">{detail.supplier.location}</div></div></div>
            <div className="grid grid-cols-3 gap-3 text-center text-[11px]">
              {[["Vendeur depuis", detail.supplier.since], ["Transactions", detail.supplier.transactions], ["Taux de réponse", detail.supplier.responseRate]].map(([label, value]) => <div key={label} className="rounded bg-gray-50 p-2"><div className="font-black text-[#0d1b2a] text-[12px]">{value}</div><div className="text-gray-400">{label}</div></div>)}
            </div>
            <div className="flex gap-2 mt-3">
              ...
              <button className="flex-1 rounded bg-[#0d1b2a] py-2.5 text-[12px] font-bold text-white hover:bg-orange-500 transition-colors">Voir le shop</button>
