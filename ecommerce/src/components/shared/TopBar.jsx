...
export default function TopBar({ backTo, backLabel = 'Retour', onBack }) {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/search?q=${encodeURIComponent(searchQuery)}&type=Produits`);
      setSearchQuery('');
    }
  };
  ...
  return (
    <div className="bg-[#0d1b2a] py-3.5 px-4">
      <div className="max-w-[1300px] mx-auto flex items-center gap-4">
        <div className="font-['Barlow_Condensed'] text-3xl font-black text-white tracking-tight shrink-0 cursor-pointer" onClick={() => navigate('/')}>
        ...
        <div className="flex items-center gap-1 shrink-0">
                ...
                {user ? (
                  <>
                    ...
                    <div className="grid divide-y divide-gray-100 border-t border-gray-100">
                      <Link to="/profile" className="flex items-center justify-between px-4 py-2.5 text-[12px] font-semibold text-gray-500 hover:bg-orange-50 hover:text-orange-500">
                        Mes informations <ChevronRight size={12} />
                      </Link>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-2 p-3">
                      <button onClick={() => navigate("/login")} className="rounded bg-orange-500 px-3 py-2 text-[12px] font-black text-white hover:bg-orange-600">
                        Se connecter
                      </button>
                      <button onClick={() => navigate("/signup")} className="rounded border border-gray-200 px-3 py-2 text-[12px] font-bold text-gray-600 hover:border-orange-300 hover:text-orange-500">
                        S'inscrire
                      </button>
                    </div>
          ...
          {actionPopovers.map(({ icon, label, title, text, primary, secondary, links }) => {
            const PopIcon = actionIconMap[icon];
            return (
              <div key={label} className="relative group">
                <button className="flex flex-col items-center gap-0.5 px-3 py-1.5 text-white rounded hover:bg-white/10 transition-colors">
                  <PopIcon size={18} className="text-orange-300" />
