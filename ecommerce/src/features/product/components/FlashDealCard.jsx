...
export default function FlashDealCard({ product, onProductClick, onAddToCart }) {
  return (
    <article onClick={() => onProductClick(product)} className="group cursor-pointer rounded-lg bg-white p-3 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="pt-3">
        ...
        <div className="mt-3 h-2 overflow-hidden rounded-full bg-gray-100">
          <div className="h-full rounded-full bg-gradient-to-r from-red-500 to-orange-400" style={{ width: `${product.sold}%` }} />
        </div>
        <div className="mt-1 text-[11px] font-bold text-gray-400">{product.sold}% du stock vendu</div>
