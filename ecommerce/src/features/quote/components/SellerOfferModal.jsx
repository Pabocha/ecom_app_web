...
export default function SellerOfferModal({ quote, onClose, sendMutation }) {
  ...
  const submitLabel = isDraft ? 'Envoyer l\'offre' : 'Mettre à jour et envoyer';
  ...
  const submit = () => {
    ...
    sendMutation.mutate({ id: quote.id, payload }, { onSuccess: onClose });
  };
      ...
      <div className="w-full max-w-[540px] max-h-[90vh] overflow-y-auto rounded-lg bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-100 p-5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-[16px] font-black text-[#0d1b2a]">Envoyer une offre</h3>
              <p className="text-[12px] text-gray-400">Configurez les détails avant d'envoyer au client</p>
            </div>
          </div>
        ...
        <div className="p-5 space-y-3">
          {lines.map((l, index) => (
            <div key={l.id ?? index} className="rounded-lg border-2 border-gray-100 p-3">
              <div className="text-[13px] font-black text-[#0d1b2a] mb-2">
                {l.product_name || `Produit #${l.product || l.variant}`}
              </div>
              ...
              <div className="mt-2">
                <label className="block text-[11px] font-bold text-gray-400 mb-1">Remarque (optionnel)</label>
                <input
                  type="text"
                  value={l.remarks}
                  onChange={(e) => update(index, 'remarks', e.target.value)}
                  placeholder="Ex : Livraison express, emballage spécial…"
                  className="w-full rounded-lg border-2 border-gray-200 px-3 py-2 text-[13px] focus:border-orange-500 focus:outline-none"
                />
              </div>
            </div>
